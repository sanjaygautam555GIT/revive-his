async function renderIPD(){
  ...
  el.innerHTML=`
    <div class="panel">
      <h2>IPD Admission V2</h2>
      <p>Search patient, admit to IPD, collect initial deposit, and maintain live admission register.</p>
    ...
    <form id="ipdForm" class="panel">
      ...
      <div class="grid" style="grid-template-columns:repeat(4,1fr)">
        ...
        <div><label>Bed No</label><input id="ipdBedNo" placeholder="Optional"></div>
        <div><label>Diagnosis / Indication</label><input id="ipdDiagnosis" placeholder="Admission diagnosis"></div>
        <div><label>Status</label><select id="ipdStatus"><option>Admitted</option><option>Discharged</option></select></div>
      </div>
    ...
    <div class="panel table-wrap">
      ...
      <table><thead><tr><th>Adm ID</th><th>UHID</th><th>Name</th><th>Department</th><th>Doctor</th><th>Ward/Bed</th><th>Treatment</th><th>Deposit</th><th>Status</th></tr></thead><tbody id="ipdRows"></tbody></table>
    </div>
  `;
...
async function saveIPDAdmission(e){
  ...
  const patientPayload={
    ...
    name,
    patient_name:name,
    age:safeNumber(document.getElementById("ipdAge").value),
    sex:document.getElementById("ipdSex").value,
    mobile,
    address:document.getElementById("ipdAddress").value.trim(),
    department:document.getElementById("ipdDepartment").value,
  ...
  const payload={
    ...
    mobile,
    address:document.getElementById("ipdAddress").value.trim(),
    admission_date:document.getElementById("ipdAdmissionDate").value,
    department:document.getElementById("ipdDepartment").value,
    doctor:document.getElementById("ipdConsultant").value.trim(),
    consultant:document.getElementById("ipdConsultant").value.trim(),
    diagnosis:document.getElementById("ipdDiagnosis").value.trim(),
    treatment_type:document.getElementById("ipdTreatmentType").value,
    ward_type:document.getElementById("ipdWardType").value,
    bed_no:document.getElementById("ipdBedNo").value.trim(),
    advance:safeNumber(document.getElementById("ipdDeposit").value),
    deposit_amount:safeNumber(document.getElementById("ipdDeposit").value),
    payment_mode:document.getElementById("ipdPaymentMode").value,
    deposit_date:document.getElementById("ipdDepositDate").value,
    status:document.getElementById("ipdStatus").value,
    remarks:document.getElementById("ipdRemarks").value.trim(),
    created_at:new Date().toISOString()
  };
...
async function loadIPDRegister(){
  ...
  body.innerHTML=rows.length?rows.map(r=>`<tr><td>${r.admission_id||r.id||""}</td><td>${r.uhid||""}</td><td>${r.patient_name||""}</td><td>${r.department||""}</td><td>${r.doctor||r.consultant||""}</td><td>${[r.ward_type,r.bed_no].filter(Boolean).join(" / ")}</td><td>${r.treatment_type||""}</td><td>${money(r.advance||r.deposit_amount||0)}</td><td>${r.status||"Admitted"}</td></tr>`).join(""):"<tr><td colspan='9'>No current IPD admissions.</td></tr>";
}
