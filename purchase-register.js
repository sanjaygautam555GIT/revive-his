...
async function renderPurchaseRegister(){
  ...
  el.innerHTML=`
    ...
    <div class="panel table-wrap">
      ...
      <table><thead><tr><th>Type</th><th>Item</th><th>Batch</th><th>Expiry</th><th>Qty</th><th>Purchase</th><th>MRP</th><th>Sale</th><th>Total</th><th>Action</th></tr></thead><tbody id="purchaseItemRows"></tbody></table>
    ...
    <div class="panel table-wrap"><h3>Recent Purchase Items</h3><table><thead><tr><th>Date</th><th>Supplier</th><th>Invoice</th><th>Type</th><th>Item</th><th>Batch</th><th>Qty</th><th>Total</th><th>Status</th></tr></thead><tbody id="purchaseRows"></tbody></table></div>`;
...
async function loadPurchaseSuggestions(){
  ...
  const pRows=purchaseRes.data||[];const sRows=stockRes.data||[];const medMap=new Map();
...
async function saveQuickSupplier(){
  const msg=document.getElementById("purchaseMessage");const name=document.getElementById("newSupplierName").value.trim();
  ...
  msg.innerHTML="<p class='success'>Supplier added.</p>";document.getElementById("quickSupplierBox").classList.add("hidden");
  ["newSupplierName","newSupplierContact","newSupplierMobile","newSupplierTerms"].forEach(id=>document.getElementById(id).value="");
  await loadPurchaseSuggestions();document.getElementById("purchaseSupplier").value=name;
}
...
function addPurchaseInvoiceItem(){
  const med=document.getElementById("purchaseMedicine").value.trim();const qty=Number(document.getElementById("purchaseQty").value||0);const purchasePrice=Number(document.getElementById("purchasePrice").value||0);const category=document.getElementById("purchaseCategory").value;
  if(!category){alert("Select item type first.");return;}if(!med){alert("Enter item name first.");return;}if(qty<=0){alert("Quantity should be more than 0.");return;}
  purchaseInvoiceItems.push({medicine_name:med,category,batch_no:document.getElementById("purchaseBatch").value.trim()||null,expiry_date:document.getElementById("purchaseExpiry").value||null,quantity:qty,purchase_price:purchasePrice,mrp:Number(document.getElementById("purchaseMrp").value||0),sale_price:Number(document.getElementById("purchaseSalePrice").value||0),total_amount:qty*purchasePrice});
  document.getElementById("purchaseMedicine").value="";document.getElementById("purchaseCategory").value="Medicine";document.getElementById("purchaseBatch").value="";document.getElementById("purchaseExpiry").value="";document.getElementById("purchaseQty").value=0;document.getElementById("purchasePrice").value=0;document.getElementById("purchaseMrp").value=0;document.getElementById("purchaseSalePrice").value=0;updatePurchaseLineTotal();renderPurchaseItems();
}
function removePurchaseItem(i){purchaseInvoiceItems.splice(i,1);renderPurchaseItems();}
function renderPurchaseItems(){
  const body=document.getElementById("purchaseItemRows");const total=purchaseInvoiceItems.reduce((s,r)=>s+Number(r.total_amount||0),0);document.getElementById("purchaseGrandTotal").textContent=money(total);
  body.innerHTML=purchaseInvoiceItems.length?purchaseInvoiceItems.map((r,i)=>`<tr><td>${r.category||""}</td><td>${r.medicine_name}</td><td>${r.batch_no||""}</td><td>${r.expiry_date||""}</td><td>${r.quantity}</td><td>${money(r.purchase_price)}</td><td>${money(r.mrp||0)}</td><td>${money(r.sale_price)}</td><td>${money(r.total_amount)}</td><td><button type="button" class="secondary" onclick="removePurchaseItem(${i})">Remove</button></td></tr>`).join(""):"<tr><td colspan='10'>No item added to current invoice.</td></tr>";
}
async function saveCompletePurchaseInvoice(){
  ...
  const supplier=document.getElementById("purchaseSupplier").value.trim();const invoiceNo=document.getElementById("purchaseInvoice").value.trim();const invoiceDate=document.getElementById("purchaseDate").value;
  if(!supplier){alert("Select supplier name first. Use + Supplier if supplier is not listed.");return;}if(!invoiceNo||!invoiceDate){alert("Fill invoice number and invoice date.");return;}if(!purchaseInvoiceItems.length){alert("Add at least one item row.");return;}
  ...
  const {error:pError}=await db.from("pharmacy_purchases").insert(rows);if(pError){msg.innerHTML=`<p class="error">Invoice save failed: ${pError.message}</p>`;return;}
  const {error:sError}=await db.from("pharmacy_stock").insert(stockRows);if(sError){msg.innerHTML=`<p class="error">Purchase saved, but stock add failed: ${sError.message}</p>`;await loadPurchases();return;}
  msg.innerHTML=`<p class="success">Invoice saved. ${rows.length} inventory items added to stock.</p>`;purchaseInvoiceItems=[];document.getElementById("purchaseInvoice").value="";document.getElementById("purchaseDate").value=todayISO();renderPurchaseItems();await loadPurchaseSuggestions();await loadPurchases();
}
async function loadPurchases(){
  const body=document.getElementById("purchaseRows");if(!body)return;body.innerHTML="<tr><td colspan='9'>Loading purchases...</td></tr>";
  const {data,error}=await db.from("pharmacy_purchases").select("*").order("created_at",{ascending:false});
  if(error){body.innerHTML=`<tr><td colspan='9' class='error'>Purchase table error: ${error.message}</td></tr>`;return;}
  const rows=data||[];document.getElementById("purchaseCount").textContent=rows.length;document.getElementById("purchaseValue").textContent=money(rows.reduce((s,r)=>s+Number(r.total_amount||0),0));document.getElementById("purchaseDue").textContent=money(rows.filter(r=>(r.payment_status||"").toLowerCase()!=="paid").reduce((s,r)=>s+Number(r.total_amount||0),0));
  body.innerHTML=rows.length?rows.slice(0,50).map(r=>`<tr><td>${r.invoice_date||rowDate(r)}</td><td>${r.supplier||""}</td><td>${r.invoice_no||""}</td><td>${r.category||""}</td><td>${r.medicine_name||""}</td><td>${r.batch_no||""}</td><td>${r.quantity||0}</td><td>${money(r.total_amount||0)}</td><td>${r.payment_status||""}</td></tr>`).join(""):"<tr><td colspan='9'>No purchase records.</td></tr>";
}
