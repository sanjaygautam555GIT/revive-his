  ...
  el.innerHTML=`
    ...
    <div class="panel">
      ...
      <div class="grid" style="grid-template-columns:repeat(4,1fr)">
        ...
        <div><label>Free / Deal</label><input id="purchaseFreeQty" type="number" value="0"></div>
    ...
    <div class="panel table-wrap">
      ...
      <table><thead><tr><th>Type</th><th>Item</th><th>Unit</th><th>Batch</th><th>Expiry</th><th>Qty</th><th>Free</th><th>Purchase</th><th>MRP</th><th>Sale</th><th>Total</th><th>Action</th></tr></thead><tbody id="purchaseItemRows"></tbody></table>
...
function addPurchaseInvoiceItem(){const med=document.getElementById("purchaseMedicine").value.trim();const qty=Number(document.getElementById("purchaseQty").value||0);const freeQty=Number(document.getElementById("purchaseFreeQty").value||0);const purchasePrice=Number(document.getElementById("purchasePrice").value||0);const category=document.getElementById("purchaseCategory").value;const unit=document.getElementById("purchaseUnit").value.trim();if(!category){alert("Select item type first.");return;}if(!med){alert("Enter item name first.");return;}if(!unit){alert("Enter unit / pack, e.g. 1x10, 1x15, 200ML.");return;}if(qty<=0){alert("Quantity should be more than 0.");return;}purchaseInvoiceItems.push({medicine_name:med,category,unit,pack:unit,batch_no:document.getElementById("purchaseBatch").value.trim()||null,expiry_date:document.getElementById("purchaseExpiry").value||null,quantity:qty,free_quantity:freeQty,purchase_price:purchasePrice,mrp:Number(document.getElementById("purchaseMrp").value||0),sale_price:Number(document.getElementById("purchaseSalePrice").value||0),total_amount:qty*purchasePrice});document.getElementById("purchaseMedicine").value="";document.getElementById("purchaseCategory").value="Medicine";document.getElementById("purchaseUnit").value="";document.getElementById("purchaseBatch").value="";document.getElementById("purchaseExpiry").value="";document.getElementById("purchaseQty").value=0;document.getElementById("purchaseFreeQty").value=0;document.getElementById("purchasePrice").value=0;document.getElementById("purchaseMrp").value=0;document.getElementById("purchaseSalePrice").value=0;updatePurchaseLineTotal();renderPurchaseItems();}
function removePurchaseItem(i){purchaseInvoiceItems.splice(i,1);renderPurchaseItems();}
function renderPurchaseItems(){const body=document.getElementById("purchaseItemRows");const total=purchaseInvoiceItems.reduce((s,r)=>s+Number(r.total_amount||0),0);document.getElementById("purchaseGrandTotal").textContent=money(total);body.innerHTML=purchaseInvoiceItems.length?purchaseInvoiceItems.map((r,i)=>`<tr><td>${r.category||""}</td><td>${r.medicine_name}</td><td>${r.unit||r.pack||""}</td><td>${r.batch_no||""}</td><td>${r.expiry_date||""}</td><td>${r.quantity}</td><td>${r.free_quantity||0}</td><td>${money(r.purchase_price)}</td><td>${money(r.mrp||0)}</td><td>${money(r.sale_price)}</td><td>${money(r.total_amount)}</td><td><button type="button" class="secondary" onclick="removePurchaseItem(${i})">Remove</button></td></tr>`).join(""):"<tr><td colspan='12'>No item added to current invoice.</td></tr>";}
