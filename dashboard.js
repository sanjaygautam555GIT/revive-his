async function loadDashboard(){
  ...
  try{
    ...
    const activeIpdAdmissions=admissions.filter(isActiveIPDAdmission);
    const ipdAdmittedToday=activeIpdAdmissions.filter(r=>dateInRange(r,"admission_date",today,today)||dateInRange(r,"created_at",today,today));
    ...
    const totalPatientsToday=opdToday.length+activeIpdAdmissions.length;
    ...
    el.innerHTML=`
      ...
      <div class="kpi-grid compact">
        ...
        ${kpiCard("Patients",totalPatientsToday,`OPD today ${opdToday.length} · IPD active ${activeIpdAdmissions.length}`,"info")}
        ...
        ${kpiCard("IPD Advance",money(todaySummary.ipdAdvanceReceived),"today only, not revenue","warning")}
      ...
      <div class="dashboard-secondary-grid">
        ...
        ${compactList("Hospital Status",[["OPD today",opdToday.length],["Active IPD",activeIpdAdmissions.length],["Admitted today",ipdAdmittedToday.length],["Pharmacy bills",pharmToday.length]])}
