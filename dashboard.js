async function loadDashboard(){
  ...
  try{
    ...
    el.innerHTML=`
      <div class="dash-head">
        <div><h2>Dashboard V4.2 – Executive Command Center</h2><p>Revive Hospital · ${new Date().toLocaleDateString()} · Updated ${new Date().toLocaleTimeString()}</p></div>
...
function lastNDaysTrend(all,n,today){
  const arr=[];
  const end=new Date(today);
  for(let i=n-1;i>=0;i--){
    const d=new Date(end);d.setDate(d.getDate()-i);
    const iso=d.toISOString().slice(0,10);
    const s=buildFinancialSummary(all,iso,iso);
    arr.push({day:iso,revenue:s.revenue,profit:s.grossProfit,expense:s.operatingExpenses});
  }
  return arr;
}
...
function miniBarChart(values,labels,title){
  const max=Math.max(...values.map(v=>Math.abs(safeNumber(v))),1);
  return `<div class="mini-chart">${values.map((v,i)=>{const h=Math.max(4,Math.round(Math.abs(safeNumber(v))/max*100));return `<div class="bar-col" title="${labels[i]} ${title}: ${money(v)}"><div class="bar ${v<0?"neg":""}" style="height:${h}%"></div><small>${i%5===0?labels[i]:""}</small></div>`}).join("")}</div>`;
}
...
function performanceScore(summary,lowStock,expired,expiring,totalPatients){
  let s=70;
  if(summary.grossProfit>0)s+=10;else if(summary.revenue>0)s-=15;
  if(summary.revenue>0)s+=10;else s-=5;
...
function ownerInsights(today,month,revenueChange,pharmacyMargin,expiring,lowStock){
  const out=[];
  out.push(`• Today revenue <b>${money(today.revenue)}</b>; net result <b>${money(today.grossProfit)}</b>.`);
  ...
  out.push(`• Pharmacy margin today is <b>${pharmacyMargin.toFixed(1)}%</b>.`);
  if(expiring.length)out.push(`• <b>${expiring.length}</b> medicine rows are expiring within 30 days.`);
  if(lowStock.length)out.push(`• <b>${lowStock.length}</b> medicine rows are low stock.`);
  out.push(`• Monthly revenue <b>${money(month.revenue)}</b>; monthly net <b>${money(month.grossProfit)}</b>.`);
  return out;
}
