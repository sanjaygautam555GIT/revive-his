async function loadDashboard(){
  ...
  try{
    const [patients,opdVisits,admissions,ipdBills,diagnosticBills,stock,pharmacySales,purchases]=await Promise.all([
      fetchAll("patient"),
      fetchAll("opd_visits"),
      fetchAll("ipd_admission"),
      fetchAll("ipd_billing"),
      fetchAll("diagnostic_bills"),
      fetchAll("pharmacy_stock"),
      fetchAll("pharmacy_sales"),
      fetchAll("pharmacy_purchases")
    ]);
    ...
    const daily={
      opdRevenue:sumField(opdToday,"amount"),
      ipdRevenue:sumField(ipdFinalToday,"total"),
      diagRevenue:sumField(diagToday,"total_amount"),
      pharmRevenue:sumField(pharmToday,"bill_amount"),
      opdCount:opdToday.length,
      ipdNew:ipdAdmissionToday.length,
      ipdActive:activeIpd.length,
      diagCount:diagToday.length,
      pharmCount:pharmToday.length,
      discharged:dischargedToday.length,
      ipdAdvance:ipdAdmissionToday.reduce((s,r)=>s+depositAmount(r),0),
      cash:collectionByMode({opd:opdToday,diag:diagToday,pharm:pharmToday,adm:ipdAdmissionToday},"cash"),
      upi:collectionByMode({opd:opdToday,diag:diagToday,pharm:pharmToday,adm:ipdAdmissionToday},"upi"),
      bank:collectionByMode({opd:opdToday,diag:diagToday,pharm:pharmToday,adm:ipdAdmissionToday},"bank")
    };
    ...
    const monthly={
      opdRevenue:sumField(opdMonth,"amount"),
      ipdRevenue:sumField(ipdFinalMonth,"total"),
      diagRevenue:sumField(diagMonth,"total_amount"),
      pharmRevenue:sumField(pharmMonth,"bill_amount"),
      opdCount:opdMonth.length,
      admissions:ipdAdmissionMonth.length,
      discharges:dischargedMonth.length,
      diagCount:diagMonth.length,
      pharmCount:pharmMonth.length,
      ipdAdvance:ipdAdmissionMonth.reduce((s,r)=>s+depositAmount(r),0),
      cash:collectionByMode({opd:opdMonth,diag:diagMonth,pharm:pharmMonth,adm:ipdAdmissionMonth},"cash"),
      upi:collectionByMode({opd:opdMonth,diag:diagMonth,pharm:pharmMonth,adm:ipdAdmissionMonth},"upi"),
      bank:collectionByMode({opd:opdMonth,diag:diagMonth,pharm:pharmMonth,adm:ipdAdmissionMonth},"bank")
    };
    ...
    const trend=lastNDaysSimple({opdVisits,ipdBills,diagnosticBills,pharmacySales},7,today);
    ...
    el.innerHTML=`
      ...
      <div class="panel"><h2>Daily Data</h2><p>Today's revenue, collection and patient activity.</p></div>
      ...
      <div class="dashboard-main-grid">
        ...
        <div class="panel"><h3>Collection Trend – Last 7 Days</h3>${miniBarChart(trend.map(r=>r.collection),trend.map(r=>r.day.slice(5)),"Collection")}</div>
      </div>
