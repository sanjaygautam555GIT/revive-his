...
async function renderPharmacyBilling(){
  ...
  el.innerHTML=`
    <div class="panel"><h2>Pharmacy Billing</h2><p>Import OPD/IPD patient and sell medicines by individual tablet, capsule, ampoule, vial or bottle.</p>
    ...
    <div class="panel"><h3>Add Item</h3><div class="grid" style="grid-template-columns:repeat(4,1fr)">
      ...
      <div><label>Sale Price per Unit</label><input id="billSalePrice" type="number" value="0" step="0.01"></div>
      <div><label>Quantity to Sell</label><input id="billQty" type="number" value="1"></div>
...
async function importPharmacyPatient(){const source=document.getElementById('phSource').value;const q=document.getElementById('phSearch').value.trim().toLowerCase();const msg=document.getElementById('phMsg');if(source==='Walk-in'){clearPharmacyPatient();msg.innerHTML="<p class='success'>Walk-in selected.</p>";return;}if(!q){msg.innerHTML="<p class='error'>Enter search text.</p>";return;}if(source==='IPD'){const rows=await fetchAll('ipd_admission');const a=rows.find(r=>isActiveAdmission(r)&&[r.admission_id,r.uhid,r.patient_name,r.mobile].join(' ').toLowerCase().includes(q));if(!a){msg.innerHTML="<p class='error'>No active IPD admission found.</p>";return;}currentPharmacyPatient={type:'IPD',ref:a.admission_id||String(a.id),uhid:a.uhid||'',name:a.patient_name||''};document.getElementById('billPatientName').value=a.patient_name||'';document.getElementById('billPatientType').value='IPD';document.getElementById('billUhid').value=a.uhid||'';document.getElementById('billRef').value=a.admission_id||String(a.id);document.getElementById('billPaymentStatus').value='Due';document.getElementById('billAmountPaid').value=0;billAmountPaidEdited=true;msg.innerHTML=`<div class='sync-box'><b>IPD patient imported</b><br>${a.patient_name||''} · ${a.uhid||''} · ${a.admission_id||''}</div>`;}else{const rows=await fetchAll('opd_visits');const v=rows.find(r=>[r.visit_id,r.uhid,r.patient_name,r.mobile].join(' ').toLowerCase().includes(q));if(!v){msg.innerHTML="<p class='error'>No OPD visit found.</p>";return;}currentPharmacyPatient={type:'OPD',ref:v.visit_id||'',uhid:v.uhid||'',name:v.patient_name||''};document.getElementById('billPatientName').value=v.patient_name||'';document.getElementById('billPatientType').value='OPD';document.getElementById('billUhid').value=v.uhid||'';document.getElementById('billRef').value=v.visit_id||'';msg.innerHTML=`<div class='sync-box'><b>OPD patient imported</b><br>${v.patient_name||''} · ${v.uhid||''} · ${v.visit_id||''}</div>`;}}
async function loadBillingStock(){const sel=document.getElementById('billStockSelect');const {data,error}=await db.from('pharmacy_stock').select('*').gt('quantity',0).order('medicine_name',{ascending:true});if(error){sel.innerHTML='<option value="">Stock load failed</option>';return;}pharmacyStockRows=data||[];sel.innerHTML='<option value="">Select stock</option>'+pharmacyStockRows.map(r=>`<option value="${r.id}">${r.medicine_name||''} | Pack ${r.unit||'1x1'} | Batch ${r.batch_no||''} | Units ${r.quantity||0}</option>`).join('');}
function selectBillingStock(){const id=document.getElementById('billStockSelect').value;const stock=pharmacyStockRows.find(r=>String(r.id)===String(id));if(!stock)return;document.getElementById('billPack').value=stock.unit||'1x1';document.getElementById('billAvailableQty').value=stock.quantity||0;document.getElementById('billSalePrice').value=Number(stock.sale_price||0).toFixed(2);updateBillLineTotal();}
...
function addPharmacyBillItem(){const id=document.getElementById('billStockSelect').value;const stock=pharmacyStockRows.find(r=>String(r.id)===String(id));if(!stock){alert('Select stock first.');return;}const qty=Number(document.getElementById('billQty').value||0);const available=Number(stock.quantity||0);if(qty<=0){alert('Quantity should be more than 0.');return;}if(qty>available){alert('Quantity is more than available individual units.');return;}const rate=Number(document.getElementById('billSalePrice').value||0);pharmacyBillItems.push({stock_id:stock.id,medicine_name:stock.medicine_name,unit:stock.unit||'1x1',batch_no:stock.batch_no,expiry_date:stock.expiry_date,purchase_price:Number(stock.purchase_price||0),sale_price:rate,quantity:qty,total:qty*rate,available_before:available});document.getElementById('billStockSelect').value='';document.getElementById('billPack').value='';document.getElementById('billAvailableQty').value='';document.getElementById('billSalePrice').value=0;document.getElementById('billQty').value=1;updateBillLineTotal();renderBillItems();}
...
function renderBillItems(){
  ...
  const grand=document.getElementById('billGrandTotal');
  ...
  body.innerHTML=pharmacyBillItems.length?pharmacyBillItems.map((r,i)=>`<tr><td>${r.medicine_name||''}</td><td>${r.unit||''}</td><td>${r.batch_no||''}</td><td>${r.expiry_date||''}</td><td>${r.quantity}</td><td>${money(r.sale_price)}</td><td>${money(r.total)}</td><td><button type="button" class="secondary" onclick="removeBillItem(${i})">Remove</button></td></tr>`).join(''):"<tr><td colspan='8'>No item added.</td></tr>";
}
...
async function loadPharmacySales(){const body=document.getElementById('salesRows');const {data,error}=await db.from('pharmacy_sales').select('*').order('created_at',{ascending:false});if(error){body.innerHTML=`