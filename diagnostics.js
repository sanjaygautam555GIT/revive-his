...
"Others":[["Others",0]]};
function isActiveDiagnosticAdmission(r){return !["discharged","final billed","cancelled","closed"].includes(String(r?.status||"Admitted").trim().toLowerCase())}
async function renderDiagnostics(){
const el=document.getElementById("diagnosticsView");
el.innerHTML=`<div class="panel"><h2>Diagnostics Billing</h2><p>Import patient, choose investigation type and name, add to bill, and save.</p></div>
<div class="panel"><h3>1. Import Patient</h3><div class="grid" style="grid-template-columns:1.5fr auto 1fr 1fr"><div><label>Search Patient / Admission</label><input id="diagPatientSearch" placeholder="Name / mobile / UHID / admission ID"></div><div><label>&nbsp;</label><button type="button" id="diagSearchBtn">Import</button></div><div><label>Billing Type</label><select id="diagBillingType"><option>OPD</option><option>IPD</option></select></div><div><label>Payment Mode</label><select id="diagPaymentMode"><option>Cash</option><option>UPI</option><option>Bank</option><option>Credit</option></select></div></div><div id="diagPatientResult"></div></div>
<div class="panel"><h3>2. Choose Investigation</h3><div class="grid" style="grid-template-columns:1fr 2fr 1fr 1fr auto"><div><label>Investigation Type</label><select id="diagTypeSelect"><option>Laboratory</option><option>ECG</option><option>X-Ray</option><option>CT Scan</option><option>Others</option></select></div><div><label>Investigation Name</label><select id="diagTestSelect"></select></div><div><label>Price</label><input id="diagItemPrice" type="number" value="0" step="0.01"></div><div><label>Qty</label><input id="diagItemQty" type="number" value="1" step="0.01"></div><div><label>&nbsp;</label><button type="button" id="addDiagItemBtn">Add</button></div></div><div id="otherDiagBox" class="hidden" style="margin-top:10px"><label>Other Investigation Name</label><input id="otherDiagName" placeholder="Enter name"></div></div>
...
populateDiagnosticNames();renderDiagnosticItems();await loadDiagnosticsRegister();}
...
async function upsertDiagnosticsLabCharge(billNo,total){
  const adm=diagnosticsState.admission;
  if(!adm)return {ok:false,message:"No IPD admission selected."};
  const admissionId=adm.admission_id||String(adm.id);
  const description=`Diagnostics Bill ${billNo}: ${diagnosticsState.items.map(i=>i.test_name).join(", ")}`;
  const payload={admission_id:admissionId,uhid:adm.uhid||"",patient_name:adm.patient_name||"",charge_date:todayISO(),category:"Lab Charge",description,rate:total,quantity:1,amount:total,created_at:new Date().toISOString()};
  const {data:existing,error:findError}=await db.from("ipd_daily_charges").select("id").eq("admission_id",admissionId).ilike("description",`%${billNo}%`).limit(1);
  if(findError)return {ok:false,message:findError.message};
  if(existing&&existing.length){
    const {error:updateError}=await db.from("ipd_daily_charges").update(payload).eq("id",existing[0].id);
    if(updateError)return {ok:false,message:updateError.message};
    return {ok:true,message:"Lab Charge updated in IPD Daily Charges."};
  }
  const {error:insertError}=await db.from("ipd_daily_charges").insert([payload]);
  if(insertError)return {ok:false,message:insertError.message};
  return {ok:true,message:"Lab Charge added to IPD Daily Charges."};
}
async function saveDiagnosticsBill(){
  ...
  if(type==="IPD"&&!diagnosticsState.admission){msg.innerHTML="<p class='error'>For IPD billing, import an active IPD admission first.</p>";return;}
  ...
  let ipdMsg="";
  if(type==="IPD"){
    const result=await upsertDiagnosticsLabCharge(billNo,total);
    if(!result.ok){msg.innerHTML=`<p class='error'>Diagnostics bill saved: ${billNo}, but Lab Charge was NOT added to IPD Daily Charges: ${result.message}</p>`;await loadDiagnosticsRegister();return;}
    ipdMsg=result.message;
  }
  msg.innerHTML=`<p class='success'>Bill saved: ${billNo}. ${ipdMsg} <button type="button" class="secondary" onclick="printDiagnosticsBill('${billNo}')">Print Bill</button></p>`;
  diagnosticsState.items=[];renderDiagnosticItems();await loadDiagnosticsRegister();
}
async function loadDiagnosticsRegister(){const body=document.getElementById("diagnosticsRegisterRows");if(!body)return;const {data,error}=await db.from("diagnostic_bills").select("*").order("created_at",{ascending:false}).limit(20);if(error){body.innerHTML=`<tr><td colspan='7' class='error'>${error.message}</td></tr>`;return;}body.innerHTML=(data||[]).length?(data||[]).map(r=>`<tr><td>${r.bill_no||""}</td><td>${r.billing_date||rowDate(r)}</td><td>${r.patient_name||""}</td><td>${r.billing_type||""}</td><td>${money(r.total_amount||0)}</td><td>${r.payment_mode||""}</td><td><button class='secondary' onclick="printDiagnosticsBill('${r.bill_no}')">Print</button></td></tr>`).join(""):"<tr><td colspan='7'>No diagnostics bills.</td></tr>";}