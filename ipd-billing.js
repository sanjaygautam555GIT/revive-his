...
function isActiveIPDBillingAdmission(r){return !["discharged","final billed","cancelled","closed"].includes(String(r.status||"Admitted").trim().toLowerCase())}
...
async function renderIPDBilling(){
  ...
  document.getElementById("ipdBillSearch").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();searchIPDBillingAdmission()}};
...
async function searchIPDBillingAdmission(){
  const q=document.getElementById("ipdBillSearch").value.trim();
  ...
  const matches=filterPatientChoices((await fetchAll("ipd_admission")).filter(isActiveIPDBillingAdmission),q,["admission_id","id","uhid","patient_name","mobile"]);
  if(!matches.length){msg.innerHTML="<p class='error'>No active IPD admission found.</p>";return;}
  renderPatientChoices(msg,matches,{recordLabel:"active admission",detailLabel:"Admission / Ward",detailValue:r=>[r.admission_id||r.id,[r.ward_type,r.bed_no].filter(Boolean).join(" / ")].filter(Boolean).join(" · ")},admission=>selectIPDBillingAdmission(admission,msg));
}

async function selectIPDBillingAdmission(admission,msg){
  ...
  msg.innerHTML=`<p class='success'>Admission loaded: ${escapePatientChoice(admission.patient_name||"Patient")}. ${ipdBillingState.items.length} daily charge(s) imported.</p>`;
...
async function importDailyChargesForBill(adm){
  ...
  const dbId=String(adm.id||"");
  const uhid=String(adm.uhid||"");
  const {data,error}=await db.from("ipd_daily_charges").select("*").order("charge_date",{ascending:true});
  if(error){ipdBillingState.items=[];return;}
  const charges=(data||[]).filter(c=>{
    const cAdm=String(c.admission_id||"");
    const cUhid=String(c.uhid||"");
    return cAdm===admissionId || cAdm===dbId || (!!uhid && cUhid===uhid);
  });
  ipdBillingState.items=charges.map(c=>({category:c.category||"Other",description:c.description||c.category||"Charge",rate:safeNumber(c.rate||c.amount),qty:safeNumber(c.quantity)||1,source:"Daily Charges"}));
}
...
function renderIPDAdmissionSummary(){
  ...
  document.getElementById("ipdAdmissionSummary").innerHTML=`
    ...
    <div class="grid" style="grid-template-columns:repeat(4,1fr)">
      <div><b>Admission ID</b><br>${a.admission_id||a.id||""}</div><div><b>UHID</b><br>${a.uhid||""}</div><div><b>Patient</b><br>${a.patient_name||""}</div><div><b>Doctor</b><br>${a.doctor||a.consultant||""}</div>
      <div><b>Department</b><br>${a.department||""}</div><div><b>Admission Date</b><br>${a.admission_date||rowDate(a)}</div><div><b>Stay</b><br>${days} day(s)</div><div><b>Advance</b><br>${money(advance)}</div>
