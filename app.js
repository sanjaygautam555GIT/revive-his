...
function navForRole(role){
  if(role==="owner")return ["dashboard","patientSearch","cashReport","reports","doctorMaster"];
  const base=(NAV_BY_ROLE[role]||[]).slice().filter(x=>x!=="dashboard"&&x!=="doctorMaster");
  ...
  if(role==="staff"&&!base.includes("printCenter")){const i=base.indexOf("diagnostics");base.splice(i>=0?i+1:base.length,0,"printCenter")}
  if(role==="pharmacyOwner"){["supplierMaster","medicineMaster"].forEach(x=>{if(!base.includes(x))base.push(x)})}
...
async function openPrintCenter(){await loadScriptOnce("print-center.js");return renderPrintCenter()}
