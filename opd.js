...
async function renderOPD(){
  ...
  document.getElementById("opdSearchTerm").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();searchOPDPatient()}};
...
async function searchOPDPatient(){
  const term=document.getElementById("opdSearchTerm").value.trim(),msg=document.getElementById("opdSearchResult");
  ...
  try{
    const matches=filterPatientChoices(await fetchAll("patient"),term,[r=>r.name||r.patient_name,"mobile",r=>r.uhid||r.patient_id]);
    if(!matches.length){msg.innerHTML="<p class='error'>No previous patient found. Enter new patient details.</p>";return;}
    renderPatientChoices(msg,matches,{detailLabel:"Address",detailValue:r=>r.address||"-"},p=>loadPatientIntoOPD(p,msg));
  }catch(error){msg.innerHTML=`<p class='error'>Search failed: ${escapePatientChoice(error.message)}</p>`;}
}
async function loadPatientIntoOPD(p,msg){document.getElementById("opdPatientId").value=p.id||"";document.getElementById("opdUhid").value=p.uhid||p.patient_id||"";document.getElementById("opdName").value=p.name||p.patient_name||"";document.getElementById("opdAge").value=p.age||"";document.getElementById("opdSex").value=p.sex||p.gender||"Male";document.getElementById("opdMobile").value=p.mobile||"";document.getElementById("opdAddress").value=p.address||"";const visits=await fetchAll("opd_visits");const uhid=p.uhid||p.patient_id||"";const patientVisits=visits.filter(v=>(uhid&&v.uhid===uhid)||(p.id&&String(v.patient_id||"")===String(p.id))||(!uhid&&!p.id&&p.mobile&&v.mobile===p.mobile));const last=patientVisits.map(v=>v.visit_date||rowDate(v)).filter(Boolean).sort().slice(-1)[0]||"-";msg.innerHTML=`<div class='sync-box'><b>Existing patient loaded</b><br>UHID: ${escapePatientChoice(uhid||"-")} &nbsp; | &nbsp; Name: ${escapePatientChoice(p.name||p.patient_name||"Patient")} &nbsp; | &nbsp; Age: ${escapePatientChoice(p.age||"-")} &nbsp; | &nbsp; Last Visit: ${escapePatientChoice(last)} &nbsp; | &nbsp; Total Visits: ${patientVisits.length}</div>`;}
