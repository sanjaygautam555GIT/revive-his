  ...
  el.innerHTML=`
    ...
    <form id="opdForm" class="panel">
      ...
      <div class="grid" style="grid-template-columns:1fr">
        <div><label>Address</label><input id="opdAddress"></div>
      </div>
...
function calculateOPDNet(){
  const fee=safeNumber(document.getElementById("opdFee")?.value);
  const discount=safeNumber(document.getElementById("opdDiscount")?.value);
  const net=Math.max(0,fee-discount);
  const el=document.getElementById("opdNetAmount");
...
function clearOPDForm(){
  ...
  document.getElementById("opdPatientId").value="";
  document.getElementById("opdUhid").value="";
...
async function loadOPDRegister(){
